...
function PartnerCard({ title, gradient, onNavigate, isSelected, id, onSelect }: PartnerCardProps) {
  const { t } = useLanguage()
...
export function PartnerView() {
  ...
  const getShareContent = () => {
    if (affiliateData?.link_share) {
      ...
      return {
        content: `${t("partner.share.content")} https://seo-ai.kz/?ref=${affiliateData.link_share}`,
  ...
  const getShareButtonStyle = () => {
    ...
    const borderClass = selectedItem === "share-button" ? "border-4 border-blue-600" : "border border-white"
  ...
  return (
    <div className="h-full  flex flex-col p-6 items-center relative overflow-hidden">
      <h1 className="text-lg sm:text-xl font-medium text-blue-600 mb-6 ml-4">{t("partner.referral.program")}</h1>
