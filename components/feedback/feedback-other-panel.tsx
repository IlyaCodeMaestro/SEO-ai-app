...
export function FeedbackOtherPanel({ onClose }: FeedbackOtherPanelProps) {
  ...
  // Находим данные для "Другое" (id: 3)
  const otherData = feedbackData?.feedback_type?.find((item) => item.id === 3);
      ...
      <div className="flex-1 px-6 py-8">
        <div className="max-w-md mx-auto bg-gray-50 rounded-lg p-6 shadow-md border dark:bg-[#2C2B2B]">
          <p className="text-gray-600 mb-6 text-center text-sm dark:text-white">
            {otherData?.note ||
              "По любым другим вопросам, не указанным в предыдущих разделах, напишите нам в мессенджер."}
          </p>
